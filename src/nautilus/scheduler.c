...
#define CO(ns) MS(ns)
    ...
    if (cpu == t->current_cpu || cpu < 0)
    {
        ...
        nk_vc_printf("%llut %lur %dc%s %s %s %s %llus %lluc %llur %llud %llue",
                     ...
                     t->status == NK_THR_INIT ? "ini" : t->status == NK_THR_RUNNING ? "RUN" : t->status == NK_THR_WAITING ? "wai" : t->status == NK_THR_SUSPENDED ? "sus" : t->status == NK_THR_EXITED ? "exi" : "UNK",
                     r->status == ARRIVED ? "arr" : r->status == ADMITTED ? "adm" : r->status == CHANGING ? "cha" : r->status == YIELDING ? "yie" : r->status == EXITING ? "exi" : r->status == SLEEPING ? "sle" : r->status == DENIED ? "den" : r->status == REAPABLE ? "rea" : "UNK",
                     CO(r->start_time),
                     CO(r->cur_run_time),
                     CO(r->run_time),
                     CO(r->deadline),
...
static void rt_queue_dump(rt_queue *queue, char *pre)
{
    ...
    for (now = 0; now < queue->size; now++)
    {
        ...
        DEBUG("   %llu %s (%llu)\n", queue->threads[cur]->thread->tid,
              queue->threads[cur]->thread->is_idle ? "*idle*" : queue->threads[cur]->thread->name[0] ? queue->threads[cur]->thread->name : "(no name)", queue->threads[cur]->deadline);
    }
    DEBUG("======%s==END=====\n", pre);
}
...
static void rt_priority_queue_dump(rt_priority_queue *queue, char *pre)
{
    ...
    for (now = 0; now < queue->size; now++)
    {
        DEBUG("   %llu %s (%llu)\n", queue->threads[now]->thread->tid,
              queue->threads[now]->thread->is_idle ? "*idle*" : queue->threads[now]->thread->name[0] ? queue->threads[now]->thread->name : "(no name)", queue->threads[now]->deadline);
    }
    DEBUG("======%s==END=====\n", pre);
}
...
static int rt_priority_queue_enqueue(rt_priority_queue *queue, rt_thread *thread)
{
    if (queue->size == MAX_QUEUE)
    {
        ERROR("Too many threads for priority queue %s\n",
              queue->type == RUNNABLE_QUEUE ? "Runnable" : queue->type == PENDING_QUEUE ? "Pending" : queue->type == APERIODIC_QUEUE ? "Aperiodic Runnable" : "UNKNOWN");
...
#define CUR_NOT_SPECIAL_STR (yielding ? "Yielding" : "Preempting")
    ...
    if (!timed_out && !apic_timer && !apic_kick && CUR_IS_NOT_SPECIAL && !yielding && !idle)
    {
...
static int select_victim(int new_cpu)
{
    ...
    return (sys->cpus[a]->sched_state->aperiodic.size >
            sys->cpus[b]->sched_state->aperiodic.size)
               ? a
               : b;
}
...
static void handle_special_switch(rt_status what, int have_lock, uint8_t flags, void (*release_callback)(void *), void *release_state)
{
    ...
    DEBUG("%sing %llu \"%s\"\n",
          what == SLEEPING ? "Sleep" : what == YIELDING ? "Yield" : what == EXITING ? "Exit" : what == CHANGING ? "Chang" : "WTF",
          c->tid, c->name);
...
static inline void rt_thread_update_aperiodic(rt_thread *t, rt_scheduler *scheduler, uint64_t now)
{
    if (t->constraints.type == APERIODIC)
    {
        ...
        {
            t->deadline = t->constraints.aperiodic.priority + MIN(t->cur_run_time, scheduler->cfg.aperiodic_quantum);
            // the priority must be bounded from below by the
            // baseline priority and from above by the idle
            // priority
            if ((t->deadline < t->constraints.aperiodic.priority) || (t->deadline > (-1ULL - 2048ULL)))
            {
...
static int rt_thread_admit(rt_scheduler *scheduler, rt_thread *thread, uint64_t now)
{
    ...
    DEBUG("Admission: %s tpr=%u util_limit=%llu aper_res=%llu spor_res=%llu per_res=%llu\n",
          thread->constraints.type == APERIODIC ? "Aperiodic" : thread->constraints.type == PERIODIC ? "Periodic" : thread->constraints.type == SPORADIC ? "Sporadic" : "Unknown",
                        ...
                        uint64_t priority)
{
    ...
    strncpy(a->name, name, SHELL_MAX_CMD);
    a->name[SHELL_MAX_CMD - 1] = 0;
